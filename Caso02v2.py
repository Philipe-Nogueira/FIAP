...
while (votantes < eleitores):
    ...
    dias_empatados = [dia for dia, votos in zip(["Segunda-Feira", "Terça-Feira", "Quarta-Feira", "Quinta-Feira", "Sexta-Feira"], votos) if
                      votos == max_votos]

if (num_max_votos == 1):
    dia_escolhido = ["Segunda-Feira", "Terça-Feira", "Quarta-Feira", "Quinta-Feira", "Sexta-Feira"][votos.index(max_votos)]
    print("A Live foi agendada para", dia_escolhido)
    imprimir_votos()
else:
    print("Houve um empate entre os dias:", ", ".join(dias_empatados))
    imprimir_votos()

"""
while (num_max_votos == 1):
